...
sealed class StandardComposedModel(val components: List<IModel>) : IModel {
  ...
  override fun bakeQuads(face: EnumFacing?, state: IExtendedBlockState) =
    components.flatMap { it.bakeQuads(face, state) }

  override fun bakeItemQuads(face: EnumFacing?, stack: ItemStack) =
    components.flatMap { it.bakeItemQuads(face, stack) }
