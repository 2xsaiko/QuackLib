...
class GuiAlloyFurnace(playerInv: InventoryPlayer, val inventory: IQBlockInventory) : GuiContainer(ContainerAlloyFurnace(playerInv, inventory)) {
  ...
  override fun drawGuiContainerBackgroundLayer(partialTicks: Float, mouseX: Int, mouseY: Int) {
    ...
    this.getBurn(13).takeUnless { it == 0 }
      ?.also { drawTexturedModalRect(windowX + 18, windowY + 30 - it, 176, 13 - it, 14, it + 1) }

    this.getProgress(22).takeUnless { it == 0 }
      ?.also { drawTexturedModalRect(windowX + 102, windowY + 33, 176, 14, it, 16) }
